...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Betsy46' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'H4dUwQnKAVMXz1i',
        'isDeleted':false,
        'username':'Betsy46',
        'email':'Alicia_Wehner98@gmail.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Betsy46' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Nat94' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'Zo4mx_HEzDKjkR2',
        'isDeleted':false,
        'username':'Nat94',
        'email':'Chad.Maggio@gmail.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Nat94' }, userToBeInserted);
    }
